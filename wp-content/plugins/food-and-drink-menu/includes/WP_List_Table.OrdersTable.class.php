			...
			case 'items' :
				...
				foreach ( $order_items as $order_item ) {
					...
					foreach ( $order_item->selected_options as $selected_option ) {
						if ( array_key_exists($selected_option, $ordering_options ) ) {
...
/*
	...
	public function orders_data() {
		...
		if( array_key_exists( 'status', $_GET ) && in_array( $_GET['status'], array_keys( $order_statuses ) ) ) {
			$status = $_GET['status'];
		}
		else {
		...
		$page = array_key_exists('paged', $_GET) ? $_GET['paged'] : 1;
		$page = $page < 1 ? 1 : $page;
	...
	private function sort_data( $a, $b ) {
	  ...
	  if(in_array($orderby, $columns)) {
	  	$orderby = array_search($orderby, $columns);
	  }
	...
	public function prepare_items() {
		...
		usort( $this->orders, array( &$this, 'sort_data' ) );
