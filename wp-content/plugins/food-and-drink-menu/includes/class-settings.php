...
<em>This message was sent by {site_link} on {current_time}.</em>',
	...
	public function load_settings_panel() {
		...
		$sap = sap_initialize_library(
			array(
				'version'		=> '2.6.1', // Version of the library
		...
		if ( ! $fdm_controller->permissions->check_permission('styling') ) {
			$styling_permissions = array(
				'disabled'		=> true,
				'disabled_image'=> '#',
				'purchase_link'	=> 'https://www.fivestarplugins.com/plugins/five-star-restaurant-menu/'
			);
		}
		...
		$sap->add_setting(
			...
			array(
				...
				'default'		=> $this->defaults['fdm-image-style-columns'],
		...
		$sap->add_setting(
			...
			array(
				...
				'default'		=> $this->defaults['fdm-related-items']
			)
		);
		$sap->add_setting(
			'food-and-drink-menu-settings',
		...
		$sap->add_section(
			...
			array(
				'id'    => 'fdm-qr-code',
				'title' => __( 'QR Code', 'food-and-drink-menu-pro' ),
				'tab'	=> 'fdm-advanced-settings'
			)
		);

		$qr_code = $fdm_controller->settings->get_setting('fdm-qr-code-url');
		$qr_code_description = __( 'Enter the URL of your menu/ordering page here and it will generate a QR code, which you can then use at your restaurant to let people view a contactless menu.', 'food-and-drink-menu-pro' );
		if ( $qr_code != '' ) {
			$qr_code_description .= '<br><br>';
			$qr_code_description .= '<a href="https://api.qrserver.com/v1/create-qr-code/?data=' . $qr_code . '&size=600x600&format=png" target="_blank"><img class="fsp-grfwp-admin-qr-code" src="https://api.qrserver.com/v1/create-qr-code/?data=' . $qr_code . '&size=600x600&format=png"></a>';
		}
		...
		$sap->add_setting(
			...
			array(
				...
				'default'		=> $this->defaults['ordering-payment-gateway'],
				'conditional_on'		=> 'enable-payment',
				'conditional_on_value'	=> true
			)
		);
		...
		$sap->add_setting(
			...
			array(
				...
				'default'		=> $this->defaults['ordering-payment-mode'],
				'conditional_on'		=> 'enable-payment',
				'conditional_on_value'	=> true
			)
		);
